...
* Controleer de logbestanden van de spelerconsole en eventuele fouten. Klik en inspecteer met de rechtermuisknop om de consolelogboeken te zien. Als u de Speler van Vensters gebruikt, druk `CTRL + ALT +I` om dev console omhoog te brengen om de logboeken te bekijken.
...
Registratie is momenteel alleen mogelijk op de auteurinstantie. Hoewel de registratieservice niet is geverifieerd, wordt alleen een apparaat in behandeling in AEM gemaakt en wordt het apparaat niet daadwerkelijk geregistreerd en wordt er geen weergave toegewezen.

Als u een apparaat wilt registreren (een gebruiker voor het apparaat in AEM maakt), moet u het verifiëren bij AEM en de registratiewizard handmatig volgen om de registratie te voltooien. In theorie kan een kwaadwillige gebruiker meerdere apparaten in behandeling maken, maar deze kan geen apparaten registreren als deze geen AEM-aanmelding hebben.

### &#x200B;2. Is er een manier om HTTP GET verzoeken in POST van HTTP met één of andere vorm van authentificatie om te zetten? {#is-there-a-way-to-transform-http-get-requests-into-http-post-with-some-form-of-authentication}

De registratieaanvraag is een POST-aanvraag.

Het wordt aanbevolen de apparaat-id op te halen uit de sessie in plaats van deze als parameter door te geven. Als u dit doet, worden de serverlogboeken, de cache van de browser enzovoort opgeschoond. Het is geen veiligheidskwestie. Semantisch. GET wordt gebruikt wanneer er geen statuswijziging op de server plaatsvindt en POST wordt gebruikt wanneer de status wordt gewijzigd.
...
Voor meer informatie over waar u apparatenactiviteit kunt controleren, zie [**de Dienst van de Berichten van AEM van Berichten**](screens-notifications-service.md).
